...
- [~] s5.2 Baselines  — harness verified; reference row B1 is complete at full counts on all four components (tool calling 0.6700 native / 0.5355 ours, structured output 0.1355, instruction following 0.8170, and a guardrail flag rate of 0.0074 with zero false alarms on the clean control arm), reads behind the 1B incumbent on tool calling, and sits at the floor on the safety axis; the competitor row B4 that carries the operative threshold is running, the 4-bit serving path is compiling after being moved to a different compute source, the harness has gained a score-only replay mode that recovers B1's lost per-item detail without regenerating, and the three GGUF rows plus B6 wait on a free GPU slot
- [ ] s5.3 Broad sweep
