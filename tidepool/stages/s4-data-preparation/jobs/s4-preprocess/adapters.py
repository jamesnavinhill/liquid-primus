...
_IMPORT = re.compile(
    ...
    r"|#include[ \t]*[<\"]([\w./]+)|require\([\'\"]([\w./@-]+)|using[ \t]+([A-Za-z_][\w.]*))", re.M)
_CALLED = re.compile(r"\b([A-Za-z_]\w*(?:\.[A-Za-z_]\w*)*)\s*\(")
_DEFINED = re.compile(r"\b(?:def|class|function|func|fn|sub|void|public|private)\s+([A-Za-z_]\w*)")


def code_symbols(answer, query=""):
    """The API surface a code answer exercises: imported modules and called symbols.

    s4.2 keyed CodeFeedback on its `resource` field, which has four values and put
    40% of the corpus in one group — unusable as a split key. The surface a row
    actually touches is a far finer key, and it is the axis the project cares
    about specializing to, so it is also the more meaningful one.
    """
    blocks = _FENCE.findall(answer or "")
    code = "\n".join(blocks) if blocks else (answer or "")
    mods, called, defined = set(), set(), set()
    for groups in _IMPORT.findall(code):
        for g in groups:
            if not g:
                continue
            for name in g.split(","):
                ...
                if name:
                    mods.add(name.split(".")[0].split("/")[0].lower())
    for m in _CALLED.findall(code):
        m = m.strip()
        if "." in m or len(m) > 2:
            called.add(m.lower())
    for m in _DEFINED.findall(code):
        defined.add(m.lower())
    return mods, called, defined


def codefeedback(row):
    """Group key is filled in by a second pass; see `code_symbols` and main.py."""
    answer = row.get("answer") or ""
    mods, called, defined = code_symbols(answer, row.get("query") or "")
    return {
        "prompt": row.get("query") or "",
        "tools": [], "calls": [], "refusal": None, "target": None,
        "group_key": None,                       # assigned by the rarity pass
        "row_id": _sha(row.get("query")),
        "schema_text": "",
        "extra": {"lang": row.get("lang"), "resource": row.get("resource"),
                  "mods": sorted(mods), "called": sorted(called)[:64],
                  "defined": sorted(defined)[:32]},
    }


def antidoom(row):
    """LiquidAI/antidoom-mix-v1.0: id, conversations, and upstream provenance.

    s4.2 read this corpus with a prompt-only adapter that looked for a `prompt`
    field, found none, and so reported one 478,229-row group with zero benchmark
    overlap. Both numbers were the reader, not the data. The prompt is the first
    human turn of `conversations`; the split key is the upstream dataset the row
    came from, which is also the unit `heldout_policy` is written against.
    """
    conv = _loads(row.get("conversations"), []) or []
    prompt, answer, n_user = "", "", 0
    for turn in conv:
        if not isinstance(turn, dict):
            continue
        frm = (turn.get("from") or turn.get("role") or "").lower()
        ...
        if frm in ("human", "user"):
            ...
            if not prompt:
                prompt = val
        elif frm in ("gpt", "assistant", "model") and not answer:
            answer = val
    src = row.get("source") or "unknown"
    conf = row.get("source_config")
    sample = row.get("source_sample_id") or _sha(prompt)
    # Group key is the upstream sample inside its source, not the source alone.
    # Keying on the source alone would make every group 2-30% of the corpus, so
    # the collapse rule would route all of them to train and leave no held-out
    # prompts at all. The leak this corpus can actually have is the same upstream
    # item arriving twice, which the sample id prevents; the source histogram is
    # reported separately so a source-disjoint split stays available to s4.4.
    return {
        "prompt": prompt,
        "tools": [], "calls": [], "refusal": None, "target": None,
        "group_key": "src:%s/%s|%s" % (src, conf if conf else "-", sample),
        "row_id": str(row.get("id") or _sha(prompt)),
        "schema_text": "",
        "extra": {"source": src, "source_config": conf,
                  "source_split": row.get("source_split"),
                  "source_license": row.get("source_license"),
                  "heldout_policy": row.get("heldout_policy"),
                  "n_turns": len(conv), "n_user_turns": n_user,
