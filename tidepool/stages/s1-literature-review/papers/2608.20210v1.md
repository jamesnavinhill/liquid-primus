...
Authors: Christos Koutsiaris Affiliation: Development Expert

## Contents
- [Abstract](#abstract1)
- [I Introduction](#S1)
  - [I-A Contributions](#S1.SS1)
  - [I-B Success criteria fixed in advance](#S1.SS2)
- [II Related Work](#S2)
- [III Architecture](#S3)
  - [III-A The short-convolution block](#S3.SS1)
  - [III-B Why six attention layers](#S3.SS2)
  - [III-C Supporting choices](#S3.SS3)
  - [III-D Rejected alternatives](#S3.SS4)
- [IV A Cost Model for CPU Decoding](#S4)
- [V Corpus and Data Pipeline](#S5)
  - [V-A Bounded repetition](#S5.SS1)
  - [V-B Packing and held-out data](#S5.SS2)
- [VI Training](#S6)
  - [VI-A Decaying the learning rate to zero](#S6.SS1)
- [VII Central Experiment: Hybrid versus Dense](#S7)
  - [VII-A Quality](#S7.SS1)
  - [VII-B Per-domain difficulty](#S7.SS2)
  - [VII-C CPU decode speed](#S7.SS3)
  - [VII-D Comparison against an external peer](#S7.SS4)
  - [VII-E Quantised artefact size](#S7.SS5)
  - [VII-F One datapoint against the bar](#S7.SS6)
- [VIII Headline Results](#S8)
- [IX Limitations](#S9)
  - [IX-A The training mixture drifted from its target](#S9.SS1)
  - [IX-B The last 8 % of training used a slightly smaller corpus](#S9.SS2)
  - [IX-C Quantisation-aware training did not run](#S9.SS3)
  - [IX-D The vocabulary is larger than this size warrants](#S9.SS4)
  - [IX-E Single seed, English only](#S9.SS5)
- [X A Negative Result: Dead Channels Cannot Be Reclaimed](#S10)
- [XI Evaluation Methodology](#S11)
  - [XI-A Quality](#S11.SS1)
  - [XI-B Decode](#S11.SS2)
- [XII Deployment Notes](#S12)
- [XIII Discussion](#S13)
- [XIV Reproducibility](#S14)
- [XV Conclusion](#S15)
- References
...
Trained from scratch on 59.9 B tokens, the model scores 47.31 on a
...
smaller 4-bit file, and decoded 1.76$\times$ faster at 2048 tokens of
...
Batch size is one. There is no batching over which to amortise weight
...
Memory bandwidth binds before compute. A contemporary CPU can issue far
...
The key–value cache is a growing tax. In an all-attention decoder,
...
*constant-size* state rather than a growing one, decode cost becomes far
...
- 1.
A concrete sub-200M architecture whose layer composition is chosen from a
CPU memory-traffic argument rather than adapted from GPU practice.
- 2.
A pre-registered, parameter-matched ablation isolating the effect of that
composition, with the decision rule fixed before either arm was scored.
- 3.
Decode measurements at three context depths against both an internal twin
and an external peer, showing the advantage grows with depth in both.
- 4.
A first-order cost model that under-predicts the measured advantage,
localising the residual to latency rather than bandwidth.
- 5.
A negative result closing structural pruning of dead convolution channels,
...
Small language models. A line of work has shown that sub-1 B models
improve far past classical compute-optimal token budgets when trained on
carefully filtered data. Pythia [6] established reproducible small
baselines; MobileLLM [7] showed that depth-over-width and embedding
...
Recurrent and hybrid sequence models. Structured state-space
models [8] and gated linear-recurrent
architectures [9] replace attention with mechanisms whose inference state
...
minority of attention layers—Griffin [10] being a prominent
...
Efficient attention. Multi-query and grouped-query
attention [3] shrink the cache by sharing key–value heads across query
...
Quantisation. Post-training quantisation to 4 bits is standard for CPU
deployment. We target the reference runtime’s [5] Q4_0
...
What is new here. The contribution is not a new operator. It is the
observation that in the batch-size-one CPU regime the *ratio* of cache-free
...
This is the crux of the design: *decoding through a convolution block costs
...
Grouped-query attention [3] with four key–value heads for twelve
...
Tied embeddings. At $49{,}152\times 768$ the embedding matrix holds
...
Feed-forward inner dimension 2048 ($2.67\times d_{\text{model}}$ rather
...
Before presenting measurements we state what the architecture *should* buy,
...
$W$ be the quantised weight bytes, $L_{A}$ the number of attention layers,
$h_{kv}$ the key–value heads, $d_{h}$ the head dimension, $b$ the bytes per cache
...
For the hybrid, $L_{A}=6$, $h_{kv}=4$, $d_{h}=64$, $b=2$, giving
$\kappa_{\text{hyb}}=6144$ B. For the dense twin ($L_{A}=24$, $h_{kv}=2$,
$d_{h}=64$), $\kappa_{\text{dense}}=12{,}288$ B — exactly twice,
despite the twin’s narrower per-layer cache, because it has four times as many
attention layers. Table [II](#S4.T2) evaluates
Eq. ([4](#S4.E4)) against measurement.
...
| Depth | $M$ hybrid | $M$ dense | Predicted | Measured |
| --- | --- | --- | --- | --- |
| 0 | 100.2 MB | 106.6 MB | 1.06$\times$ | 1.20$\times$ |
| 512 | 103.3 MB | 112.8 MB | 1.09$\times$ | 1.45$\times$ |
| 2048 | 112.8 MB | 131.7 MB | 1.17$\times$ | 1.76$\times$ |

The model is directionally right and quantitatively wrong, and the
...
two effects that Eq. ([4](#S4.E4)) does not model. First, attention traverses
...
*larger* than a naive byte count suggests, and that improving attention
kernels would narrow but not close the gap—the cache the hybrid does not keep
cannot be optimised.

## V Corpus and Data Pipeline

Training data is a ten-source English mixture totalling 16.93 B unique
tokens, weighted toward educational and reasoning-dense text
(Table [III](#S5.T3)). At this scale data quality dominates quantity, and the
...
data [2]. Mass freed by a capped source is redistributed by
water-filling across sources retaining headroom.

The cap is load-bearing. Without it, a source such as
*everyday-conversations* (approximately 400 k tokens) would be repeated
...
ids. Each source reserves whole shard *files*—never token slices—for a
...
Table [IV](#S6.T4) summarises the configuration. Parameters are split by
tensor shape: two-dimensional weight matrices are optimised by Muon [4],
...
| Learning rates | Muon 0.02, AdamW $3\times 10^{-4}$ |
...
The schedule is warmup–stable–decay with *linear decay to zero*, not
cosine decay to a floor, following evidence that full decay to zero is
substantially more sample-efficient [1].

A practical consequence deserves emphasis for anyone reading the loss curve:
training loss is expected to plateau during the stable phase. Between
...
|  | val_bpb $\downarrow$ | 5-task |
| --- | --- | --- |
| Daedalus-150M (hybrid) | 0.910398 | 44.68 |
| dense-150m (twin) | 0.917774 | 44.82 |
...
The honest summary is that the hybrid matches on quality and wins
decisively on decode. It is not a downstream-task win, and is not presented as
one.

### VII-B Per-domain difficulty

Table [VI](#S7.T6) decomposes validation bits-per-byte by source for the
...
| 2048 | $\mathbf{739.3\pm 35.5}$ | $\mathbf{420.3\pm 5.5}$ | 1.76$\times$ |

Table [VII](#S7.T7) contains the central result, and its *shape* is the
thesis. At depth zero the hybrid has almost nothing to gain, because its
advantage *is* the cache it does not keep and an empty context has no cache
...
| Depth | Daedalus | Peer-135M | Ratio |
| --- | --- | --- | --- |
| 0 | $960.9\pm 3.4$ | $908.1\pm 37.1$ | 1.06$\times$ |
| 512 | $933.7\pm 28.5$ | $625.7\pm 38.5$ | 1.49$\times$ |
| 2048 | $\mathbf{648.6\pm 12.6}$ | $\mathbf{312.4\pm 7.2}$ | 2.08$\times$ |
...
95.56 MiB against the twin’s 101.62 MiB—6.3 % smaller, at 4.99
...
only 5 B tokens—scores 44.7 on the five-task harness. That clears the
...
Table [IX](#S8.T9) gives the finished model measured on the harness described
in Section [XI](#S11), alongside the peers it was required to beat.
...
speed at a fixed size, and Section [VII-C](#S7.SS3) is the other half of that
trade.
...
Validation bits-per-byte over a 645 M-token held-out set is 0.8685.
...
against 9.75 at Q4_0, measured on held-out encyclopedic text). That is
...
Measured as $L_{1}$ distance in percentage points, the run as a whole is at
10.42 against a limit of 10.0 that was fixed in advance. The largest
...
The released model is therefore quantised after training rather than
...
embedding table holds 37.7 M parameters — 23 % of the model — where
...
out of the quantised file gave zero mismatches against the reference
...
saving. This was tested and does not work. The reference runtime creates
...
check_tensor_dims: expected 3,768, got 3,640, while both the
...
length-normalised accuracy where the harness specifies it. Crucially, *peer
...
*byte*, rather than per token, is used so that the figure is comparable
...
*alternate within a single pass* rather than running back to back: an
...
Quantisation format. Q4_0 is chosen over formats with better
...
Q4_K would reduce the error and give back part of the speed.

Memory footprint. The shipped artefact is 95.56 MiB. At 2048-token
...
Thread scaling. All figures here use 8 threads. Because decoding at
...
When the hybrid wins. The advantage is a function of context depth. Any
deployment that keeps a long conversation, retrieves documents into the prompt,
or processes files will sit at the right-hand end of Table [VII](#S7.T7).
Deployments that issue short, independent prompts into an empty context will see
the depth-0 row and little benefit.

Extrapolation beyond 2048. Eq. ([4](#S4.E4)) implies the ratio keeps
...
Future work. Diagnosing the quantisation-aware training failure;
addressing dead channels at initialisation rather than export; a depth ablation
($18\times 768$ against $24\times 640$) which is designed but unrun; multi-seed
...
downstream tasks, a 6.3 % smaller 4-bit file, and 1.76$\times$ faster
...
At full scale the model scores 47.31 against a bar of 42.20 set before
...
- [1]
K. Bergsma *et al.*, “Straight to zero: Why linearly decaying the learning
rate to zero works best for LLMs,” *arXiv:2502.15938*, 2025.
- [2]
N. Muennighoff *et al.*, “Scaling data-constrained language models,”
*Advances in Neural Information Processing Systems*, 2023,
arXiv:2305.16264.
- [3]
J. Ainslie *et al.*, “GQA: Training generalized multi-query transformer
models from multi-head checkpoints,” *Proc. EMNLP*, 2023,
arXiv:2305.13245.
- [4]
K. Jordan *et al.*, “Muon: An optimizer for hidden layers in neural
networks,” 2024.
- [5]
G. Gerganov *et al.*, “llama.cpp: LLM inference in C/C++,” 2023.
[Online]. Available: [https://github.com/ggml-org/llama.cpp](https://github.com/ggml-org/llama.cpp)
- [6]
S. Biderman *et al.*, “Pythia: A suite for analyzing large language models
across training and scaling,” *Proc. ICML*, 2023, arXiv:2304.01373.
- [7]
Z. Liu *et al.*, “MobileLLM: Optimizing sub-billion parameter language
models for on-device use cases,” *Proc. ICML*, 2024, arXiv:2402.14905.
- [8]
...
*Findings of EMNLP*, 2023, arXiv:2305.13048.
- [10]
S. De *et al.*, “Griffin: Mixing gated linear recurrences with local
attention for efficient language models,” *arXiv:2402.19427*, 2024.